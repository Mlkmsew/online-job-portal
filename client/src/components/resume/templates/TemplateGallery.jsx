  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        <div>
          <p className="text-sm font-semibold uppercase tracking-[0.3em] text-blue-600">Resume Builder</p>
        ...
        <div className="relative w-full max-w-sm">
          ...
          <input
            ...
            className="w-full rounded-full border border-slate-200 bg-slate-50 py-2.5 pl-10 pr-4 text-sm text-slate-700 outline-none transition focus:border-blue-400 focus:bg-white"
          />
        </div>
      </div>

      <div className="flex flex-wrap items-center gap-2">
        {Object.entries(getColorTokens()).length > 0 && ['blue', 'purple', 'black'].map((color) => {
        ...
        >
          {filtered.map((template) => {
            ...
            return (
              <div
                  key={template.id}
                  className={`w-[240px] md:w-[260px] flex-none group rounded-[24px] border bg-white p-4 shadow-sm transition-all duration-200 hover:-translate-y-1 hover:shadow-xl ${isSelected ? 'border-blue-500 shadow-[0_20px_40px_rgba(37,99,235,0.16)]' : 'border-slate-200'}`}
                >
                <div className="flex flex-wrap items-center justify-between gap-2">
                  <span className={`rounded-full px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.24em] ${template.badge === 'Premium' ? 'bg-sky-50 text-sky-700' : 'bg-blue-50 text-blue-700'}`}>
                    {template.badge}
                  </span>
                ...
                <div className="mt-4 flex items-start justify-between gap-3">
                  ...
                  {isSelected && <div className="rounded-full bg-blue-100 p-2 text-blue-700"><FiCheck /></div>}
                </div>
