    ...
    <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
      <div className="rounded-3xl border border-emerald-200 bg-emerald-50 p-5">
        <div className="flex flex-wrap items-center justify-between gap-3">
          ...
          <div className="rounded-full border border-emerald-300 bg-white px-3 py-1 text-sm font-medium text-emerald-700">Hiring Decision Hub</div>
        </div>
      </div>
        ...
        {actions.map((action) => (
          <button key={action.title} type="button" className="rounded-2xl border border-slate-200 bg-slate-50 p-4 text-left">
            <div className="flex items-center gap-2 text-sm font-semibold text-slate-900">
              {action.title === 'Move to Hired' && <CheckCircle2 className="h-4 w-4 text-emerald-600" />}
